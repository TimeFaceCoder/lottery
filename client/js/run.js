...
var readLottery = function(cb) {
  ...
  async.waterfall(tasksQueue, function() {
    cb && cb()
  })
};
...
var animation = function(id) {
  ...
  $(domId).addClass("doingAnimate").animate({
    top: "-=" + (bodyHeight + picHeight) + "px"
  }, {
    duration: 3000,
    complete: function() {
      $(domId).css("top", 0 + "px");
  ...
  showWinnerList([].concat(queue), function() {
    service.postEmployeeList(queue, function(data) {
      ...
      var html = template({
        ...
        pro1: productList[0],
        name: productionName[productList[0]]
      });
